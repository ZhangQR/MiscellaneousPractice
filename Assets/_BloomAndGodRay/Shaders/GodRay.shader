Shader "URPPractice/GodRay"
{
	...
	fixed4 fragExtractBright(v2fExtractBright i) : SV_Target
	{
		...
		// _ViewPortLightPos.xy 的原点跟 uv 一致
		// return fixed4(_ViewPortLightPos.xy,0,1);

		float distanceControl = saturate(_LightRadius - distFromLight);

		// 构造一个以 lightTransform 为圆心，中间最亮，逐渐变暗的圆。根据分辨率不同，会变成椭圆
		// return fixed4(distanceControl,distanceControl,distanceControl,1);
